...
class ClientOnlineTest extends TestCase {
    /**
     * @var Client
     */
    public static $api;
    ...
    public static function setUpBeforeClass() {
        self::$api = new Client((new ClientConfig())
            ->setHandle(getenv('SHOPIFY_API_DOMAIN'))
            ->setAccessToken(getenv('SHOPIFY_API_PASSWORD'))
            ->setMaxAttemptsOnServerErrors(3)
            ->setMaxAttemptsOnRateLimitErrors(10)
        );
    }
    ...
    public function testChangingApiVersion() {
        ...
        $apiVersion = self::$api->getConfig()->getApiVersion();
        $regex = '/^(\d{4})-(\d{2})$/';
        $this->assertRegExp($regex, $apiVersion);

        $nextApiVersion = $this->getNextApiVersion($apiVersion);
        self::$api->getConfig()->setApiVersion($nextApiVersion);
        self::$api->get('products/count');
